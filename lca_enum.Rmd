---
title: "Latent Class Analysis Enumeration"
...
This walkthrough is presented by the IMMERSE team and will cover latent class enumeration steps and data visualizations.
...
library(hrbrthemes)nevermin
library(poLCA)
here::i_am("run_lca_enumeration.Rmd")
```
...
We are considering the use of the CRDC data repository as an ongoing example dataset which would be used to demonstrate mixture modeling in training program and online resource components of the IES grant. The Civil Rights Data Collection is a federally mandated school-level data collection effort that occurs every other year. This public data is currently available across 4 years (2011, 2013, 2015, 2017) and all US states. We are interested specifically in 6 variables which report on harassment/bullying in schools based on disability, race, or sex. We have only recently become aware of this data resource and are interested in the reputation of this data source and whether it is considered reliable among policy researchers. 
...
tribble(
   ~"Name",      ~"Label",  ~"Values",                                   
#--------------|--------------------------------|-----|,
  "report_dis",   "lied to avoid taking an exam"                   ,  "0 = No, 1 = Yes",
  "report_race",  "lied to avoid handing a term paper in on time",  "0 = No, 1 = Yes",
  "report_sex", "purchased a term paper to hand in as their own or had obtained a copy of an exam prior to taking the exam"     ,  "0 = No, 1 = Yes",
  "counselors_fte",   "copied answers during an exam from someone sitting near to them ",  "0 = No, 1 = Yes") %>% 
  gt() %>% 
  tab_header(
...
```{r, eval=TRUE}
df_bully <- read_csv(here("data", "crdc_hs_az17.csv")) %>% 
  clean_names() %>% 
...
plot_lca_function(model_name = output_bully$c2_bully.out)
```
...
```{r, eval = FALSE}
ggsave(here("figures", "C2_bully_LCA_Plot.png"), dpi="retina", height=5, width=7, units="in")
```
